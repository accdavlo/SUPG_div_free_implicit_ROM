...
class MOR:
    ...
    def run_offline(self, mu_offline, load_sol=False):
        ...
        self.basis = {}
        Sigma_svd = dict()
        self.basis["u"], Sigma_svd["u"], _ = np.linalg.svd(us, full_matrices=False)
        self.basis["v"], Sigma_svd["v"], _ = np.linalg.svd(vs, full_matrices=False)
        self.basis["p"], Sigma_svd["p"], _ = np.linalg.svd(ps, full_matrices=False)
