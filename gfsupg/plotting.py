...
def plot_all_sols(problem, FEM2D, q_save, it, time, levels=None, tol_colorbar=1e-13):
    ...
    for k, var in enumerate(problem.vars):
        axs[k].set_title(var)
        if it is None:
            level_list = np.linspace(q_save[var][:].min() - tol_colorbar, \
                                     q_save[var][:].max() + tol_colorbar, levels) if isinstance(levels,int) else levels
            plot_sol(FEM2D,q_save[var][:], axs[k],fig, level_list) 
        else:
            level_list = np.linspace(q_save[var][it,:].min() - tol_colorbar, \
                                     q_save[var][it,:].max() + tol_colorbar, levels) if isinstance(levels,int) else levels
